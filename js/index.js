function loadSkills(skills){
		...
		while(i<skills.length){
			...
			for(j=i;j<i+6&&j<skills.length;j++){
				var skill = '<div class="col m2"><img src="'+skills[j].icon+'"></path></img>'+skills[j].name+'</div>';
				row+=skill;
...
function loadWorks(experience){
	experience.sort(function(a,b){
		return a.sn-b.sn;
	});
	var i;
	var works = experience.filter((experience)=>experience.type=="work");
	var worksInnerHTML = '';
...
function loadEducations(educations){
	...
	for(i=0;i<educations.length;i++){
		education = '<div class="row education"><div class="col m6 s12">					<div class="row title">'+educations[i].course+'<hr></div><div class="row">'+educations[i].periodStart+'-'+educations[i].periodEnd+'</div><div class="row">'+educations[i].inst+'</div><div class="row">'+educations[i].board+'</div>	</div><div class="col m6 s12 details"><ul class="collapsible" data-collapsible="accordion"><li><div class="collapsible-header"><i class="material-icons">view_list</i>Completed Core Courses</div><div class="collapsible-body">';
		...
		for(j=0;j<courses.length;j++){
				coursesInnerHTML+='<div class="row"><div class="col m2 s2">'+courses[j].courseCode+'</div><div class="col m10 s12">'+courses[j].courseName+'</div></div>';
		}
...
swal({
		title: "Hello World!!!",
		text: "Hello Visitor, you have landed upon the webspace of Kaustav. I hope you're doing well. \n\n I'm always on the lookout for fantastic SDE role opportunities. Are you hiring? ",
	...
	if(value===true) {
		swal({
			...
			text: "Thanks a lot for visiting my webspace. I hope you will find pertinent information here. If you need any other information, kindly reach me, by either dropping a mail or giving me a call. \n\n Do you need a copy of my resume?",
			buttons: {
				cancel: {
				  text: "I already have your resume",
				  value: false,
				  visible: true,
				  className: "button-cancel",
				  closeModal: true,
			...
			if(value===true) { 
				window.open('/Kaustav_Sarkar_Resume.pdf');
			}
		})
	}
});
...
$.get("js/profile.json", 
	function(data, status){
		...
		$('title').html(pInfo.fname+' '+pInfo.lname+'|Portfolio');
